// ==============================================
// GERLNO UNIVERSE 30-IN-1 PLUGINS (SINGLE FILE)
// ==============================================
...
(function() {
  ...
  abtn.innerText = '🎵';
  abtn.style.cssText = 'position:fixed;top:20px;right:400px;padding:8px 12px;background:rgba(0,0,0,.3);color:#fff;border:1px solid #fff3;border-radius:10px;cursor:pointer;z-index:999';
  document.body.appendChild(abtn);
  let playing = false;
  abtn.onclick = () => {
    playing ? audio.pause() : audio.play();
    playing = !playing;
    abtn.innerText = playing ? '⏸️' : '🎵';
  };
  ...
  findBtn.innerText = '🔎';
  findBtn.style.cssText = 'position:fixed;top:20px;right:440px;padding:8px 12px;background:rgba(0,0,0,.3);color:#fff;border:1px solid #fff3;border-radius:10px;cursor:pointer;z-index:999';
  ...
  fold.innerText = '📂';
  fold.style.cssText = 'position:fixed;top:20px;right:480px;padding:8px 12px;background:rgba(0,0,0,.3);color:#fff;border:1px solid #fff3;border-radius:10px;cursor:pointer;z-index:999';
  ...
  speed.style.cssText = 'position:fixed;bottom:20px;right:20px;color:#fff8;background:rgba(0,0,0,.3);padding:6px 10px;border-radius:8px;font-size:12px;z-index:999';
  document.body.appendChild(speed);
  setInterval(async () => {
    const t1 = performance.now();
    await fetch('https://speed.cloudflare.com/__down?bytes=100000');
    const t2 = performance.now();
    const b = 100000 * 8 / (t2-t1) / 1000;
    speed.innerText = b.toFixed(1) + ' Mbps';
  }, 3000);
